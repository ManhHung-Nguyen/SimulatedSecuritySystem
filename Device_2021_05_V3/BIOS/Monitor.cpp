...
class LCD {
	void drawRow(int y, int left, int mid, int right = 0);
	void clear_gui();
public:
	LCD() {}
	void init();
...
Monitor& Monitor::operator<<(const char* s) {
	lcd.print(s);
	return *this;
}
...
Monitor& Monitor::operator()(int line) {
	lcd.setCursor(0, line);
	return *this;
}
Monitor& Monitor::operator()(int x, int y) {
	lcd.setCursor(x, y);
	return *this;
}
...
void Monitor::WakeUp(int timeout) {
	lcd.light();
}
void Monitor::Sleep() {
	lcd.light(false);
